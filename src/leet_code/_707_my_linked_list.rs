...
impl MyLinkedList {
    ...
    fn add_at_head(&mut self, val: i32) {
        ...
        let mut curr =
            Rc::new(RefCell::new(ListNode { val, prev: self.head.clone(), next: next.clone() }));
    ...
    fn add_at_tail(&mut self, val: i32) {
        ...
        let mut curr =
            Rc::new(RefCell::new(ListNode { val, prev: prev.clone(), next: self.head.clone() }));
    ...
    fn add_at_index(&mut self, index: i32, val: i32) {
        ...
        curr.as_ref().unwrap().borrow_mut().prev.as_ref().unwrap().borrow_mut().next =
            Some(node.clone());
