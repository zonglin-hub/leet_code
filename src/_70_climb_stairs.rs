...
impl Solution {
    ...
    /// 这是采用动态规划实现的爬楼梯方法：
    ///
    /// - 我们可以用一个dp数组来存储到达每个台阶的方法数，其中dp[i]表示到达第i个台阶的方法数。
    /// - 当n=1时，只有一种方法，即爬1级楼梯，所以dp[1]=1。
    /// - 当n=2时，有两种方法，即爬1级楼梯两次或者直接爬2级楼梯，所以dp[2]=2。
    /// - 对于n>2的情况，每次可以选择爬1级或2级楼梯，到达第n个台阶的方法数等于到达第(n-1)个台阶的方法数加上到达第(n-2)个台阶的方法数，即dp[n]=dp[n-1]+dp[n-2]。
    /// - 最后返回dp数组的第n个元素即可。
    ///
    ...
    pub fn climb_stairs_v3(n: i32) -> i32 {
        ...
        // });
        //
        // 这里是使用 Rust 语言的 for_each 方法进行遍历，这个方法接受一个闭包作为参数，然后对 2 到 n 这些数进行遍历，对于每个数 i，都执行闭包中的代码。
        // 闭包中的代码就是将 dp[i] 的值设为 dp[i-1] 和 dp[i-2] 的值之和。
        // 这里使用了数组下标来访问 dp 数组的元素，因为数组下标必须是整数类型，所以需要将 i 转换为 usize 类型。而 (i-1) 和 (i-2) 的类型都是 i32，所以也需要将它们转换为 usize 类型，才能正确访问 dp 数组中的元素。
